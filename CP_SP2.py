...
def CP_opt(job_list,setup_time,machines,tc_list,dc):
    ...
    #for i in range(1,job_number+1):
            ...
            #for j in range(1,machine_number+1):
               # mdl.add(makespan>=end_of(x[i,j]))
    #for i in range(1, job_number + 1):
    #    if job_list[i-1].agent!="C":
    #        model.add_constraint(sum(z[i,l] for l in range(1,dc+1))==1)
    ...
    p2 = search_phase(
        vars=job.values())
    search_array = [p2,p1]
    model.set_search_phases(search_array)
...
for ins in instances:
    ...
    start = time.time()
    siter = model.start_search(TimeLimit=100)
    end = time.time()
    ...
    for i in range(len(ins)):
        if ins[i].agent == "A":
            trd += last_result.get_value(tar[i + 1])
    end = time.time
    obj = siter.get_last_result().get_objective_value()
    best_time = None
    best_obj = 10000000

    for sol in siter:
        if abs(sol.get_objective_value() - best_obj) < 1e-4:
            best_obj = sol.get_objective_value()
            best_time = round(sol.get_solve_time(), 3)
...
file_path = 'C:/Users/eduran/OneDrive - University College Cork\Desktop/exact_methods/results_cp2.txt'
