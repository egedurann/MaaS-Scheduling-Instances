...
def cp_model(job_list, setup, machines, v, x, z_tot, tc,dc):
    ...
    for i in range(jobs_no + 1):
        for j in range(1, machines + 1):
            if i == 0:
                i_k.append((0, j))
            elif i > 0 and x[i, j].solution_value >= 0.9:
                i_k.append((i, j))
    ...
    for i in i_k:
        if i[0] == 0:
            a = interval_var(name='job_{}'.format(i), start=0, end=(0, 20000), size=0)
            job[i] = a
        else:
            epsilon = v[i].solution_value - int(v[i].solution_value)
            a = interval_var(name='job_{}'.format(i), start=(0, 20000), end=(0, 20000),
    ...
    for j in range(1, machines + 1):
        ...
        jobs_on_machine_j = [i for i in range(jobs_no + 1) if (i, j) in i_k]
        matrix = [[setup[j - 1][i1][i2] for i2 in jobs_on_machine_j] for i1 in jobs_on_machine_j]
        seq = sequence_var([job[i, j] for i in jobs_on_machine_j])
        sequence[j] = seq
...
for ins in instances:
    ...
    if instances.index(ins) + 1 ==10:
        ...
        mdl, x, F, v, ben, i_n_k, c_b, T, z_tot, z = math_model(ins, s[instances.index(ins)], machines[instances.index(ins)],tc[instances.index(ins)],dc)
        stop = False
        start = time.time()
        count = 0
        print(s[instances.index(ins)])
        while stop == False:
            count += 1
            ...
            if Time_Left > 0.4:
                ...
                for i in i_n_k:
                    if ben[i].solution_value >= 0.95:
                        ben_arr.append(i)
                model, Tar, w = cp_model(ins, s[instances.index(ins)], machines[instances.index(ins)], v, x, z_tot, tc[instances.index(ins)],dc)
            ...
            if Time_Left > 0.4:
                ...
                for i in range(1, len(ins) + 1):
                    for k in range(1, machines[instances.index(ins)] + 1):
                ...
                mdl.add_constraint(c_b >= sl[w] - mdl.sum(
                    ...
                    k in range(1, machines[instances.index(ins)] + 1) for n in range(1, ins[i - 1].amount + 1)))
                for i in range(1, len(ins) + 1):
                    if ins[i - 1].agent == "A":
                        mdl.add_constraint(T[i] >= sl[Tar[i]] - sl[Tar[i]] * mdl.sum(
                            (1 - ben[i, n, k]) for k in range(1, machines[instances.index(ins)] + 1) for n in
            ...
            if F_vk - 0.001 <= F.solution_value:
                ...
                results_benders.append({
                    ...
                    "Machines": machines[instances.index(ins)],
                    "DC": dc,
                    "Obj": F_vk,
                    "Tardiness": trd,
                    "Makespan": ms,
...
with open(file_path, 'w', newline='') as file:
    ...
    for result in results_benders:
        writer.writerow([
            result['Instance'], result['Machines'], result["DC"], result["Obj"],result['Tardiness'],result["Makespan"], result["Solve Time"], result["iteration"],
